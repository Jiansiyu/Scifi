...
//  T->Add(Form("./raw_test_Rootfiles/acsis_%d.root",run_number));
  ...
  for(Int_t z=0; z<nch; z++){
    ...
    tt1[z] = new TH1F(Form("tt1_%d",z),Form("Ped %d",z),5000,8000,25000);
  }
...
//     //  TH1F *tt1 = new TH1F("tt1",Form("Ped%d",i+1),300,300,630);
    ...
    // if(k<10){
    T->Draw(Form("R.sf.A[%d]>>tt1_%d",k,k));

    cout << Form("R.sf.A[%d]>>tt1[%d]",k,k) << endl;
    // }
    // else{
    //   T->Draw(Form("R.sf.A[%d]>>tt1[%d]",k,k));
    // }
