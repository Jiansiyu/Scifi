...
//  T->Add(Form("/adaqfs/home/a-onl/tritium_work/MARANAL0/replay/Rootfiles/tritium_%d*.root",run_number));
  ...
  for(i=0;i<nch;i++){
    c1->cd(i+1);
    tt1->Reset();
    T->Draw(Form("L.sf.A[%d]>>tt1",i),tritype);
    tt1->SetXTitle(Form("L.sf.A[%d]",i));
...
//    delete tt1;
 ...
 //w   cout << "Lcer" << i << endl;
  ...
  for(i=0;i<10;i++){
    c2->cd(i+1);
    T->Draw(Form("L.sf.A_p[%d]>>tt2",i),tritype);
    tt2->SetXTitle(Form("L.sf.A_p[%d]",i));
  ...
  for(i=0;i<10;i++){
    c3->cd(i+1);
    T->Draw(Form("FbusL.sf.A[%d]>>tt3",i),tritype);
    tt3->SetXTitle(Form("FbusL.sf.A[%d]",i));
