...
/*Double_t xmin[24]={-1.20,-1.11,-1.01,-0.89,-0.80,-0.70,-0.60,-0.50,-0.40,-0.30,-0.20,-0.08,0.001,0.10,0.20,0.30,0.40,0.50,0.60,0.70,0.80,0.90,1.0,1.09};
Double_t xmax[24]={-1.09,-0.99,-0.91,-0.81,-0.71,-0.61,-0.51,-0.41,-0.31,-0.21,-0.11, -0.009,0.09, 0.19,0.29,0.39,0.49,0.59,0.69,0.79,0.89,0.99,1.09,1.19};
Double_t ps_ymin[2] = {-0.35,0.01};
Double_t ps_ymax[2] = {-0.08,0.35};


Double_t Sh_xmin[15] = {-1.14,-0.99,-0.83,-0.68,-0.53,-0.37,-0.22,-0.07,0.08,0.24,0.39,0.54,0.69,0.85,1.00};
Double_t Sh_xmax[15] = {-1.00,-0.84,-0.69,-0.54,-0.38,-0.23,-0.08,0.07,0.23,0.38,0.53,0.68,0.84,0.99,1.14};
Double_t Sh_ymin[5] =  { -0.38,-0.22,-0.06,0.08,0.24 };
Double_t Sh_ymax[5] =  { -0.23,-0.08,0.07,0.23,0.38  };
*/
...
void Shower_1() { 
  ...
  // JW : changed so N and Y are defined outside if conditions
  const int N = 48;
  const int Y = 75;
  ...
  TString ped_name1=Form("%sHRS_SHOWER_Ped_%d.pdf[",arm,runNo[0]);
  TString ped_name2=Form("%sHRS_SHOWER_Ped_%d.pdf",arm,runNo[0]);
  TString ped_name3=Form("%sHRS_SHOWER_Ped_%d.pdf]",arm,runNo[0]);
  ...
  for (Int_t i=0;i<48;i++){
    ...
    TFitResultPtr fit_1 = ps_ped->Fit("gaus", "S", "", x1-ped_fit_range, x1+ped_fit_range);
    TFitResultPtr fit_2 = ps_ped->Fit("gaus", "S", "", (fit_1->GetParams())[1]-2.*(fit_1->GetParams())[2], (fit_1->GetParams())[1]+ 2.*(fit_1->GetParams())[2]);
    delete htmp;
    
    ped_ps[i]=fit_2->GetParams()[1];
    sigma_ps[i] = fit_2->GetParams()[2];
    gPad->SetTickx();
    gPad->SetLogy();
    gPad->SetGridx();
  ...
  for (Int_t i=0; i<75;i++){
    ...
    TFitResultPtr sh_Fit_1 = sh_ped->Fit("gaus", "S", "", x2-ped_fit_range2, x2+ped_fit_range2);
    TFitResultPtr sh_Fit_2 = sh_ped->Fit("gaus", "S", "", (sh_Fit_1->GetParams())[1]-2.*(sh_Fit_1->GetParams())[2], (sh_Fit_1->GetParams())[1]+2.*(sh_Fit_1->GetParams())[2]);
    delete htmp2;
	  
    ped_sh[i]=sh_Fit_2->GetParams()[1];
    sigma_sh[i] = sh_Fit_2->GetParams()[2];
    gPad->SetTickx();
    gPad->SetLogy();
    gPad->SetGridx();
    ...
    //		delete ps_ped; JW: moved to previos for-loop
    delete sh_ped;
	}
  ...
  for (Int_t i_evt=0; i_evt<EventNo;i_evt++) {  
    ...
    //if ((scer[0]>2000)&&(e1[0]+e2[0])/p[0]/1000>0.7&&(n_tr==1)&&(chi2[0]/ndof[0]<40.)){
    if ((scer[0]>2000)&&(e1 + e2)/p[0]/1000>0.7&&(n_tr>0)){
	...
	for (Int_t j=ps_block;j<=ps_block+1;j++){
      ...
      else if (ps_block>46 && ps_block<48){
	...
	for (Int_t j=ps_block-1;j<=ps_block;j++){
      ...
      else if ((ps_block>0 && ps_block<22)){
	for (Int_t j=ps_block-1;j<=ps_block+1;j++){
      ...
      else if ( ( ps_block>24 && ps_block<47)){
	for (Int_t j=ps_block-1;j<=ps_block+1;j++){
	...
	else if (sh_block==14||sh_block==29||sh_block==44 ||sh_block==59){
	  //  if (sh_block==29||sh_block==44){
	  for (Int_t i= sh_block-1;i<=sh_block;i++){
	    //cout <<"2"<<"\t"<<sh_block<<endl;
	    if(sh[i] -ped_sh[i]>5){Ai[i+48] = (sh[i] - ped_sh[i])/1000.;}
	    if(sh[i+15] -ped_sh[i+15]>5){Ai[i+48+15] = (sh[i+15] - ped_sh[i+15])/1000.;} //-15 is if we skip column 0 
	    //if(sh[i-15] -ped_sh[i-15]>0){Ai[i-48-15] = (sh[i-15] - ped_sh[i-15])/1000.;}
	  }
	}
	
	      
	else if (sh_block>60 && sh_block<74){
	...
	*/
	...
	// JW: added extra brackets around and conditons in OR conditions s.t. it made logical sense (and could compile), unsure if correct
	else if (sh_block>15 && sh_block<59 && ( sh_block!= 29 || sh_block!=30 || sh_block!=44 || sh_block!=45)){ 
