...
//	for (Int_t i_evt=0; i_evt<1000000;i_evt++) {  
		...
		if( n_tr == 1 && n_u1_clust == 1 && n_u2_clust == 1 && n_v1_clust == 1 && n_v2_clust == 1 &&
			...
			Int_t nMultPS = nMultPreShower;
			for(Int_t n_ps = 0; n_ps < nMultPS; n_ps++) {
				Int_t blockNum = ps_nBlk[n_ps];
...
void RHRS_minuit_cluster_cut1() {
	...
	T->Add("/home/tyler/tritium/RHRS_calibration/rootfiles/tritium_*.root");
	
	numEvents = T->GetEntries();

	// read the pedestals
	ifstream preshowerPedFile("ped_preshower.dat");
	ifstream showerPedFile("ped_shower.dat");
	Double_t preShowPed;
	Double_t showPed;
	...
	ofstream fit_preshower("minuit_cluster_cut1.ps.dat");
	ofstream fit_shower("minuit_cluster_cut1.sh.dat");
	cout << setprecision(5);
