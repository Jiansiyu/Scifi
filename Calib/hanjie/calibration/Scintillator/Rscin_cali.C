...
//  T->Add(Form("/chafs1/work1/tritium/Rootfiles/tritium_%d*.root",run_number));
  ...
  Double_t gain[16];
  ofstream myfile;
  myfile.open("Rscin_ped.txt",fstream::app);
  ...
  for(i=0;i<16;i++){
    c1->cd(i+1);
    TH1F *tt1 = new TH1F("tt1",Form("FADC_Rs2_la%d",i+1),800,1000,6500);
    T->Draw(Form("R.s2.la[%d]>>tt1",i,i),tritype);
...
//    delete tt1;
 //w   cout << "Lcer" << i << endl;
  ...
  for(i=0;i<16;i++){
    c2->cd(i+1);
    TH1F *tt2 = new TH1F("tt2",Form("Rs2_lap_%d",i+1),300,-200,1000);
    T->Draw(Form("R.s2.la_p[%d]>>tt2",i,i),tritype);
    tt2->SetXTitle(Form("R.s2.la_p[%d]",i));
    tt2->GetXaxis()->SetRangeUser(0,140);
    min_bin = tt2->GetMinimumBin();
    min_bin = tt2->GetXaxis()->GetBinCenter(min_bin);
    tt2->GetXaxis()->SetRangeUser(-200,1000);

    TF1 *f1 = new TF1("f1", "landau",min_bin, min_bin+2*fscale);
    ...
    lfgain[i]=170*1.0/landau->GetParameter(1);
...
//    delete tt2;
  ...
  for(i=0;i<16;i++){
    c3->cd(i+1);
    TH1F *tt3 = new TH1F("tt3",Form("FADC_Rs2_ra%d",i+1),800,2000,6500);
...
//    delete tt3;
  ...
  for(i=0;i<16;i++){
    c4->cd(i+1);
    TH1F *tt4 = new TH1F("tt4",Form("Rs2_rap_%d",i+1),300,-200,1000);
    T->Draw(Form("R.s2.ra_p[%d]>>tt4",i,i),tritype);
    tt4->SetXTitle(Form("R.s2.ra_p[%d]",i));
    tt4->GetXaxis()->SetRangeUser(0,140);
    ...
    rfgain[i]=170*1.0/landau->GetParameter(1);
