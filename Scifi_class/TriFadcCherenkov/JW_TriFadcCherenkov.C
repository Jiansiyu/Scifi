...
Int_t TriFadcCherenkov::ReadDatabase( const TDatime& date )
{
  ...
  // OpenFile defined in THaAnalysisObject: uses GetDBFileName() which returns fPrefix as the name of the database file (unsure how this is defined)
  // GetDBFFileName() calls GetPrefix()
  // ReadDatabase is called by Init() (which is called on detector objects) function called in analyzer (calls InitModules() in DoInit() function)
  // This Init() is inherited from THaAnalysisObject and calls MakePrefix()
  // MakePrefix() uses GetName() + "." (if given no parameters) (if overridden in an inherited class (ie here) it could give better name) must be case here. 

  // read sdk class: fPrefix is actually name of spectrometer (ie L or R), and fName is name of object

  // name is given in cosntructor of class: 
  ...
  DBRequest config_request[] = {
    { "detmap",  &detmap,  kIntV },
    { "npmt",    &nelem,   kInt },
    { "angle",   &angle,   kDouble, 0, 1 }, // first zero means accepting if there is no angle in the database file
    { 0 }
  };
  ...
  //JW: means first flag is used first (5th parameter) and second flag as 6th parameter (see ThaDetMap for details)
...
Int_t TriFadcCherenkov::Decode( const THaEvData& evdata )
{
  ...
  for( Int_t i = 0; i < fDetMap->GetSize(); i++ ) {
    // this goes through all the modules (which can contain many channels) in the detmap


    THaDetMap::Module* d = fDetMap->GetModule( i );
    bool adc = (d->model ? fDetMap->IsADC(d) : i < fDetMap->GetSize()/2 );
    // think this line just checks if the THaDetMap::Module has a model data member:
    // if it does it chekcs if this is ADC
    // if not it checks if its in the 1st half of fDetMap (read lines above about presuming first half are ADCs


    if(adc) fFADC = dynamic_cast <Fadc250Module*> (evdata.GetModule(d->crate, d->slot));
    //JW: casts module (Decoder::Module) to a Fadc class 


    // JW: Loop over all channels that have a hit (GetNumChan get no of channels with hits)
    for( Int_t j = 0; j < evdata.GetNumChan( d->crate, d->slot ); j++) {

      Int_t chan = evdata.GetNextChan( d->crate, d->slot, j );
      // JW: GetNextChan must get next channel with a hit
      if( chan < d->lo || chan > d->hi ) continue;     // Not one of my channels (JW: d contains module info including high and low channels)

      // Get the detector channel number, starting at 0
      Int_t k = d->first + ((d->reverse) ? d->hi - chan : chan - d->lo) - 1;
      // JW: condition changes if 'logical' channels run against 'physcial' channel numbers (ThaDetMap.cxx has explanation)
