...
void channel_ped_track_midev(Int_t run = -1, Int_t channum = 10, Double_t events = 10001){
  ...
  if ( run == -1 ){
    ...
    std::cout << ".x channel_ped_track.cc(#run, #channum(def=0), #events(def=10000) )" << std::endl;
	...
	std::cout << "\n" <<"  Macro goes through chosen run and channel and caluclates pedestal for every 1000 bins, it then plots the event number against pedestal. The number of events gone through is controlled by input parameter #events."  <<  std::endl;
	return 1;
	}
	else{
	  std::cout << "Do you want description of macro? (y/n)" << std::endl;
  ...
  for (Int_t j=0; j<nch; j++){
    ...
    sprintf(hname,"raw_distrib_2D%d",j);
    ...
    gr1_m[j]->GetXaxis()->SetTitle("Group number");
