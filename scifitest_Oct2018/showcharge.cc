...
void showcharge(int runnum = 5039,
		int pflag = 0,
		int outflag = 0)
{
  const int nch = 64; // The number of channels
  //  pflag = 1;      // Prit flag (you want to print? yes:1) 
  ...
  for(int i=0 ; i<nch ; i++){
    ...
    hist1[i]->GetXaxis()->SetRangeUser(-500.0, 3000.0);
  }
  ...
  if(outflag==1){
    char outfname[500];
    sprintf(outfname,"./singlePE/intcHist_acsis_%d.root",run);
