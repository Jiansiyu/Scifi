...
void conv1(int runnum = 4600, int dataflag = 0){
  ...
  // =============================== //
  char peddata[500];
  //sprintf(chmapdata,"ped/pedestal_5042.dat");
  sprintf(chmapdata,"ped/pedestal_5047.dat");
  ifstream* dragon_ped = new ifstream(chmapdata);
  double pedch[nch];
  double ttemp2;
  for(int i=0 ; i<nch ; i++){
    *dragon_ped >> ttemp2 >> pedch[i] >> ttemp2;
    //cout << pedch[i] << endl;
    //pedch[i] = 0.0;
  }
  dragon_ped->close();
  ...
  for(int i=0 ; i<nch ; i++){
    char name[500];
    sprintf(name,"Ndata.sbs.hcal.a.raw%.2d",i);
    t1->SetBranchAddress(name,&nn[i]);
    sprintf(name,"sbs.hcal.a.raw%.2d",i);
  ...
  for(int i=0 ; i<ent ; i++){
	...
	hph[j]->SetBinContent(k+1,ph[j][k]-pedch[j]); // after pedestal subtraction
	...
	if(0<tempbin && tempbin<n){
      ...
      // ----- Integrated charge ------- //
      intc[j] = hph[j]->Integral(intrange_min,intrange_max);
      hph[j]->Clear(); // reset 
    }
    tnew->Fill();
  }
